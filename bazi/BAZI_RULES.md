...
**输出对象：** 只输出"用神十神大类"
- 对每个十神大类（印/财/官杀/食伤/比劫），计算该大类对应五行（相对日主）
- 若该五行 ∈ 用神候选，则该大类进入"六亲助力"框
- 即使原局该大类总力量=0，也必须输出（并标注原局没有）

**括号状态：**
- `total_percent == 0` → `（原局没有XX星）`
- `total_percent > 0`：根据子类占比显示（纯正/纯偏/混杂）
...
**强度话术分档：**
- `total_percent == 0` → "该助力有心帮助但能力一般；走到{大类}运/年会有额外帮助。"
- `0 < total_percent < 30` → "用神有力，助力较多。"
- `total_percent >= 30` → "用神力量很大，助力非常非常大。"

**名称字段：**
- 大多数大类用自身名称：印/财/官杀/食伤/比劫
- 仅对"比劫"在原局有星时（total>0）：第1行名称打印为"比肩"
- 若比劫原局没有星（total==0），名称仍打印"比劫"

**来源清单：**
- 印：正印/偏印/混杂/原局没有的不同来源
- 比劫：比肩/劫财/混杂/原局没有，统一使用同一套来源文案
- 食伤：食神/伤官/混杂/原局没有（原局没有时包含"技术"）
- 财：正财/偏财/混杂/原局没有，区分男命/女命（男命包含"妻子/老婆/伴侣"）
- 官杀：正官/七杀/混杂/原局没有，区分男命/女命（女命包含"老公/丈夫/男友"）
...
**总计：25个 regression 用例（随功能增加而更新）**
- main函数中运行：16个（基础规则 + 合类/婚配/原局打印等）
- main函数后单独运行：9个（黄金回归7个 + 原局问题2个）
